...
## 布尔条件
condition = arr > 5 # 生成布尔数组
print(f"大于5的条件: {condition}")

## 布尔索引
filtered = arr[arr > 5] # 使用布尔索引过滤
...
arr = np.random.randint(1, 10, size=5)  # 生成5个1到10之间的随机整数
print(f"原始整数数组: {arr}")
# 修改数组类型为 float
...
prod = np.prod(arr)  # 计算所有元素的乘积
print(f"所有元素乘积: {prod}")

sum = np.sum(arr)  # 计算所有元素的和
print(f"所有元素的和: {sum}")

all_true = np.all(arr > 0)  # 检查是否所有元素都大于0
print(f"所有元素都大于0: {all_true}")

any_true = np.any(arr > 8)  # 检查是否有元素大于8
print(f"是否有元素大于8: {any_true}")

count_nonzero = np.count_nonzero(arr > 5)  # 计算大于5的元素个数
print(f"大于5的元素个数: {count_nonzero}")

diffs = np.diff(arr)  # 计算相邻元素的差值
print(f"相邻元素差值: {diffs}")

rounded = np.round(arr / 3, 2)  # 除以3并四舍五入到2位小数
print(f"除以3并四舍五入: {rounded}")

cumprod = np.cumprod(arr)  # 计算累积乘积
print(f"累积乘积: {cumprod}")

cumsum = np.cumsum(arr)  # 计算累积和
print(f"累积和: {cumsum}")

exp = np.exp(arr)  # 计算e的幂
print(f"e的幂: {exp}")

log = np.log(arr)  # 计算自然对数
print(f"自然对数: {log}")

sqrt = np.sqrt(arr)  # 计算平方根
print(f"平方根: {sqrt}")
