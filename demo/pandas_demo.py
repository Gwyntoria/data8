...
# ! Definitions and relationships of `Series` and `DataFrame`
# * Series: A one-dimensional data structure, which can be seen as a one-dimensional array with labels.
# *     Consists of two parts: index (labels) + data (values).
# *     Similar to a Python dictionary (keys == index, values == data), but with fixed order and support for vectorized operations.
# * DataFrame: A two-dimensional data structure, which can be seen as a table formed by combining multiple Series by columns.
# *     Row index + column index.
# *     Each column is essentially a Series.
# *     Similar to a spreadsheet or SQL table, supporting complex data operations and analysis.

# * DataFrame is a container of Series:
# *     Each column of a DataFrame == a Series (sharing the same row index).
# *     A DataFrame is essentially an "ordered dictionary of Series".

# ! CRUD (Create, Read, Update, Delete)
# * Add a new column by direct assignment: df["new_col"] = ...
# * Add a new row using pd.concat([...])

# * Delete a column using df.drop(columns=[...])
# * Delete a row using df.drop(index=[...])

# * For data modification, it is recommended to use loc for indexing to avoid the SettingWithCopyWarning.
# *     df.loc[row_condition] is used to query rows, where row_condition is a boolean index (e.g., a Series generated by df["col"] > val).
# *     df.loc[df["col"] > val, "col"] = new_val
# *     The first parameter is the row selection condition, and the second parameter is the column name.
# *     Batch modification of column data can use vectorized operations or the apply function.
# *     Batch modification of row data can combine loc and boolean indexing.
# *     Avoid directly assigning to filtered results (chained assignment); use loc instead.

# ! Adding and modifying will change the original DataFrame unless inplace=False is used.
# ! Deleting returns a new DataFrame by default without modifying the original df, unless inplace=True.
